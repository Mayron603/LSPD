...
// Mais seguro: coloque no .env como VITE_DISCORD_WEBHOOK_URL=seu_webhook
const DISCORD_WEBHOOK_URL = import.meta.env.VITE_DISCORD_WEBHOOK_URL || 'COLE_SEU_LINK_AQUI';

const formatMoney = (value) => `$${Number(value || 0).toLocaleString('pt-BR')}`;
const normalizarQtd = (valor) => {
...
export default function CalculadoraPenal() {
  ...
  const [oficial, setOficial] = useState(usuarioSalvo.nome || '');
  ...
  const [fiancaPaga, setFiancaPaga] = useState(false);
  const [relatorioTexto, setRelatorioTexto] = useState('');
  const [enviando, setEnviando] = useState(false);

  const crimesFiltrados = BANCO_CRIMES_REBAIXADOS.filter(
    (c) =>
  ...
  crimes.forEach((crime) => {
    const qtd = normalizarQtd(crime.qtd);

    if (crime.pena === 'BAN') {
      temBan = true;
    } else {
      penaBaseCalculada += (Number(crime.pena) || 0) * qtd;
    }

    multaTotal += (Number(crime.multa) || 0) * qtd;

    if (crime.fianca === 'NAO') {
      inafiancavel = true;
    } else {
      fiancaTotal += (Number(crime.fianca) || 0) * qtd;
    }
  });

  const podePagarFianca = !inafiancavel && !temBan && fiancaTotal > 0;
  const totalAPagar = multaTotal + (podePagarFianca && fiancaPaga ? fiancaTotal : 0);
  const displayPenaBase = temBan ? 'BANIMENTO' : penaBaseCalculada;
  const displayFianca = inafiancavel ? 'INAFIANÇÁVEL' : formatMoney(fiancaTotal);
  const displayPenaFinal = temBan ? 'BANIMENTO' : reuPrimario ? Math.floor(penaBaseCalculada * 0.7) : penaBaseCalculada;

  useEffect(() => {
    if (!podePagarFianca && fiancaPaga) setFiancaPaga(false);
  }, [podePagarFianca, fiancaPaga]);
  ...
  const handleAddItem = () => {
    if (qtdItem > 0 && itemSelecionado.trim() !== '') {
      setItens([...itens, { nome: itemSelecionado.trim().toUpperCase(), qtd: normalizarQtd(qtdItem) }]);
      setItemSelecionado('');
      setQtdItem(1);
    }
  };
  ...
  const handleAddCrime = (crime) => {
    setCrimes([...crimes, { ...crime, qtd: 1 }]);
    setBuscaCrime('');
    setMenuCrimesAberto(false);
  };

  const atualizarQtdCrime = (index, valor) => {
    const qtd = normalizarQtd(valor);
    setCrimes(crimes.map((crime, i) => (i === index ? { ...crime, qtd } : crime)));
  };
  ...
  const gerarRelatorioTexto = () => {
    ...
    const crimesTxt =
      crimes.length > 0
        ? crimes
            .map((c) => {
              const qtd = normalizarQtd(c.qtd);
              const penaTxt = c.pena === 'BAN' ? 'BAN' : `${Number(c.pena) * qtd}m`;
              const multaTxt = formatMoney((Number(c.multa) || 0) * qtd);
              return `• ${qtd}x ${c.artigo} - ${c.nome} (${penaTxt} | MULTA: ${multaTxt})`;
            })
            .join('\n')
        : 'NENHUM';
...
[SENTENÇA FINAL]:
> PENA BASE: ${displayPenaBase} ${!temBan ? 'MESES' : ''}
> MULTA TOTAL: ${formatMoney(multaTotal)}
> FIANÇA: ${displayFianca}
> FIANÇA PAGA: ${podePagarFianca ? (fiancaPaga ? 'SIM' : 'NÃO') : 'NÃO APLICÁVEL'}
...
\`\`\``);
  ...
  const enviarDiscord = async () => {
    ...
    if (DISCORD_WEBHOOK_URL === 'COLE_SEU_LINK_AQUI' || !DISCORD_WEBHOOK_URL) {
      alert('Erro: O Link do Webhook não foi configurado. Configure VITE_DISCORD_WEBHOOK_URL no .env.');
      return;
    }
    ...
    const crimesDiscord = crimes
      .map((c) => {
        const qtd = normalizarQtd(c.qtd);
        const penaTxt = c.pena === 'BAN' ? 'BAN' : `${Number(c.pena) * qtd} Meses`;
        const multaTxt = formatMoney((Number(c.multa) || 0) * qtd);
        return `- ${qtd}x [${c.artigo}] ${c.nome} (${penaTxt} | ${multaTxt})`;
      })
      .join('\n');
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('Falha ao enviar webhook. Verifique se o link do Discord está correto.');
      }
    } catch (error) {
      console.error(error);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <div className="flex items-center gap-3 text-blue-400 font-black uppercase tracking-[0.35em] text-xs mb-2">
              <Terminal size={18} /> TERMINAL LSPD-OS v5.1 // CÓDIGO PENAL
            </div>
            <h1 className="text-3xl md:text-5xl font-black text-white uppercase tracking-tight flex items-center gap-3">
              <Calculator className="text-blue-500" /> Registro de Ocorrência
            </h1>
          </div>

          <button
            onClick={limparTudo}
            className="bg-red-950/40 hover:bg-red-900/70 border border-red-900/60 text-red-300 font-black px-5 py-3 rounded-xl uppercase tracking-widest text-xs flex items-center justify-center gap-2 transition-all"
          >
          ...
          <div className="space-y-6">
            <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-6 backdrop-blur-xl shadow-xl">
              <h3 className="text-xs font-black text-blue-400 uppercase tracking-widest mb-4 flex items-center gap-2 border-b border-slate-800 pb-2">
                <Activity size={16} /> Envolvidos e Local
              </h3>
            ...
            <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-6 backdrop-blur-xl shadow-xl">
              ...
              <div className="space-y-4">
                <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-4 transition-all">
                  ...
                  {temVeiculo && <input type="text" className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2 mt-2 text-white focus:border-blue-500 outline-none font-mono text-sm placeholder:font-sans placeholder:text-slate-600" value={placa} onChange={(e) => setPlaca(e.target.value)} placeholder="PLACA (EX: ABC-1234)" />}
                </div>
                ...
                <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-4 transition-all hover:border-red-900/50">
                  ...
                  {temRefem && <input type="text" className="w-full bg-slate-900 border border-red-900/50 rounded-lg px-4 py-2 mt-2 text-white focus:border-red-500 outline-none text-sm" value={nomeRefem} onChange={(e) => setNomeRefem(e.target.value)} placeholder="Nome do Refém ou Descrição" />}
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-6 backdrop-blur-xl shadow-xl">
              ...
              <div className="flex gap-2 mb-4">
                <input type="text" list="lista-itens-ilegais" className="flex-1 bg-slate-950/50 border border-slate-800 rounded-xl px-4 py-2 text-white text-sm outline-none focus:border-orange-500 placeholder:text-slate-600 uppercase" placeholder="Selecione ou digite um item..." value={itemSelecionado} onChange={(e) => setItemSelecionado(e.target.value.toUpperCase())} />
                <datalist id="lista-itens-ilegais">
                  {ITENS_ILEGAIS.map((item) => <option key={item} value={item} />)}
                </datalist>
                <input type="number" min="1" className="w-20 bg-slate-950/50 border border-slate-800 rounded-xl px-3 py-2 text-white text-sm outline-none focus:border-orange-500" value={qtdItem} onChange={(e) => setQtdItem(normalizarQtd(e.target.value))} />
                <button type="button" onClick={handleAddItem} className="bg-orange-600 hover:bg-orange-500 text-white font-black px-4 rounded-xl text-xs uppercase tracking-widest transition-all">ADD</button>
              </div>

              <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-4 min-h-[90px] mb-4">
                {itens.length === 0 ? (
                  <p className="text-slate-600 text-xs font-bold uppercase tracking-widest text-center py-6">Lista de Itens Vazia</p>
                ) : (
                  itens.map((item, idx) => (
                    <div key={idx} className="flex justify-between items-center bg-slate-900 rounded-lg px-3 py-2 mb-2 border border-slate-800">
                      <span className="text-sm font-bold text-slate-200">{item.qtd}x {item.nome}</span>
                      <button type="button" onClick={() => handleRemoverItem(idx)} className="text-slate-500 hover:text-red-500 transition-colors p-1"><Trash2 size={14} /></button>
                    </div>
                  ))
                )}
              </div>

              <label className="block border-2 border-dashed border-slate-700 hover:border-blue-500 rounded-2xl p-6 text-center cursor-pointer transition-all bg-slate-950/40">
                <input type="file" accept="image/*" className="hidden" onChange={handleImageChange} />
                {previewImagem ? (
                  <div className="space-y-3">
                    <img src={previewImagem} alt="Evidência" className="max-h-48 mx-auto rounded-xl border border-slate-700" />
                    <p className="text-xs font-black text-blue-400 uppercase tracking-widest">Imagem Anexada ao Relatório</p>
                  </div>
                ) : (
                  <div className="flex flex-col items-center gap-2 text-slate-500">
                    <Upload size={24} />
                    <p className="text-xs font-black uppercase tracking-widest">Anexar Foto das Apreensões</p>
                    <p className="text-[10px] uppercase tracking-widest">Será enviada junto ao Webhook</p>
                  </div>
                )}
              </label>

              {previewImagem && (
                <button type="button" onClick={removerImagem} className="mt-3 text-red-400 hover:text-red-300 text-xs font-black uppercase tracking-widest flex items-center gap-2">
          ...
          <div className="space-y-6 flex flex-col relative">
            <div className="relative z-30 overflow-visible bg-slate-900/80 border border-slate-800 rounded-3xl p-6 backdrop-blur-xl shadow-xl">
              ...
              <div className="relative">
                ...
                <input type="text" className="w-full bg-slate-950/80 border border-slate-700 rounded-xl pl-12 pr-12 py-4 text-white text-lg focus:border-emerald-500 outline-none transition-all placeholder:text-slate-600 cursor-pointer" placeholder="Pesquisar ou selecionar crime..." value={buscaCrime} onChange={(e) => { setBuscaCrime(e.target.value); setMenuCrimesAberto(true); }} onFocus={() => setMenuCrimesAberto(true)} onBlur={() => setTimeout(() => setMenuCrimesAberto(false), 200)} />
                <div className="absolute inset-y-0 right-0 pr-4 flex items-center cursor-pointer" onClick={() => setMenuCrimesAberto(!menuCrimesAberto)}>
                      ...
                      crimesFiltrados.map((crime, idx) => (
                        <div key={idx} onClick={() => handleAddCrime(crime)} className="p-4 border-b border-slate-700 hover:bg-emerald-900/40 cursor-pointer flex justify-between items-center transition-colors">
                          ...
                          <div className="text-right">
                            ...
                            <span className="block text-emerald-500 text-xs font-mono">{formatMoney(crime.multa)}</span>
                          </div>
                        </div>
                      ))
                    ) : (
                      <div className="p-6 text-center text-slate-400 text-sm font-bold uppercase tracking-widest">Nenhum crime corresponde à pesquisa.</div>
                    )}
                  </div>
                )}
              </div>
              ...
              <div className="bg-slate-950/80 border border-slate-800 rounded-xl flex-1 p-4 overflow-y-auto custom-scrollbar min-h-[150px]">
                {crimes.length === 0 ? (
                  ...
                  crimes.map((crime, idx) => {
                    const qtd = normalizarQtd(crime.qtd);
                    return (
                      <div key={idx} className="grid grid-cols-1 sm:grid-cols-[1fr_auto_auto] gap-3 sm:items-center bg-slate-900 border-l-4 border-l-red-500 border-y border-r border-slate-800 rounded-lg p-3 mb-3">
                        <div>
                          ...
                          <div className="mt-1 text-xs font-mono text-slate-400 sm:hidden">
                            {crime.pena === 'BAN' ? 'BANIMENTO' : `${Number(crime.pena) * qtd} Meses`} • {formatMoney((Number(crime.multa) || 0) * qtd)}
                          </div>
                        </div>

                        <div className="flex items-center gap-2">
                          <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">QTD</span>
                          <input type="number" min="1" value={qtd} onChange={(e) => atualizarQtdCrime(idx, e.target.value)} className="w-20 bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-center text-white font-mono font-black outline-none focus:border-red-500" />
                        </div>

                        <div className="flex items-center gap-4 justify-between sm:justify-end">
                          <div className="text-right hidden sm:block">
                            <span className={`text-sm font-mono font-bold block ${crime.pena === 'BAN' ? 'text-red-500' : 'text-slate-300'}`}>{crime.pena === 'BAN' ? 'BANIMENTO' : `${Number(crime.pena) * qtd} Meses`}</span>
                            <span className="text-emerald-500 text-xs font-mono block">{formatMoney((Number(crime.multa) || 0) * qtd)}</span>
                          </div>
                          <button type="button" onClick={() => handleRemoverCrime(idx)} className="bg-slate-950 border border-slate-700 hover:bg-red-900/50 hover:text-red-400 text-slate-400 px-3 py-1.5 rounded-lg text-xs font-black transition-colors uppercase">Remover</button>
                        </div>
                      </div>
                    );
                  })
              ...
              {!temBan && (
                <div className="mt-4 bg-emerald-950/20 border border-emerald-900/50 rounded-xl p-4 flex items-center justify-between cursor-pointer hover:bg-emerald-900/30 transition-colors" onClick={() => setReuPrimario(!reuPrimario)}>
                  ...
                  <div className={`w-12 h-6 rounded-full p-1 transition-colors duration-300 ${reuPrimario ? 'bg-emerald-500' : 'bg-slate-800'}`}>
                    <div className={`w-4 h-4 rounded-full bg-white transition-transform duration-300 ${reuPrimario ? 'translate-x-6' : 'translate-x-0'}`} />
                  </div>
                </div>
              )}

              {podePagarFianca && (
                <div className="mt-4 bg-yellow-950/20 border border-yellow-700/40 rounded-xl p-4 flex items-center justify-between cursor-pointer hover:bg-yellow-900/20 transition-colors" onClick={() => setFiancaPaga(!fiancaPaga)}>
                  <div>
                    <p className="text-yellow-400 font-bold uppercase tracking-widest text-sm">Fiança paga?</p>
                    <p className="text-yellow-100/50 text-xs mt-0.5">Se ativado, a fiança entra no Total a Pagar.</p>
                  </div>
                  <div className={`w-12 h-6 rounded-full p-1 transition-colors duration-300 ${fiancaPaga ? 'bg-yellow-500' : 'bg-slate-800'}`}>
                    <div className={`w-4 h-4 rounded-full bg-white transition-transform duration-300 ${fiancaPaga ? 'translate-x-6' : 'translate-x-0'}`} />
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              ...
              <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 text-center shadow-lg">
                ...
                <p className="text-xl md:text-2xl font-mono font-bold text-emerald-500">{formatMoney(multaTotal)}</p>
              ...
              <div className={`border rounded-2xl p-4 text-center shadow-lg ${inafiancavel ? 'bg-red-900/20 border-red-500/30' : 'bg-slate-900/80 border-slate-800'}`}>
                ...
                <p className={`text-lg pt-1 font-mono font-bold ${inafiancavel ? 'text-red-500' : 'text-yellow-500'}`}>{displayFianca}</p>
                <p className="text-[10px] text-slate-600 uppercase mt-1">{fiancaPaga ? 'Paga' : 'Não paga'}</p>
              </div>

              <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 text-center shadow-lg">
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Total a Pagar</p>
                <p className="text-xl md:text-2xl font-mono font-bold text-yellow-400">{formatMoney(totalAPagar)}</p>
                <p className="text-[10px] text-slate-600 uppercase mt-1">Multa + Fiança</p>
              </div>
              ...
              <div className={`border rounded-2xl p-4 text-center shadow-lg relative overflow-hidden ${temBan ? 'bg-red-600/20 border-red-500/50' : 'bg-blue-600/10 border-blue-500/30 shadow-[0_0_20px_rgba(37,99,235,0.2)]'}`}>
                <div className={`absolute inset-0 bg-gradient-to-t ${temBan ? 'from-red-600/20' : 'from-blue-600/20'} to-transparent`} />
